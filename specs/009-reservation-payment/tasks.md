...
- [ ] T01 — Agregar a `Reservation`: campos `pendingTransferAmount`/
...
- [ ] T02 — Migración `V7__add_reservation_payment_fields.sql` +
      `ReservationEntity`/`ReservationRepositoryAdapter` con los dos campos nuevos ·
      repo: backend · depende de T01 · ~25 min

  *(fin lote 1: T01-T02)*

- [ ] T03 — `RegisterPaymentCommand`/`RegisterPaymentUseCase`/`RegisterPaymentService` +
...
- [ ] T04 — `PaymentController` (endpoints `POST .../payments`,
