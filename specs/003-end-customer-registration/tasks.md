...
- [x] T04 — Puerto `RegisterCustomerUseCase` + `RegisterCustomerCommand`; agrega `existsByTenantIdAndEmail` a `MembershipRepositoryPort` · repo: backend · ~15 min · depende de T02
- [x] T05 — Adaptador de persistencia: agrega `first_name`/`last_name`/`phone` a `MembershipEntity` y al `save()` de `MembershipRepositoryAdapter`; agrega `existsByTenantIdAndEmail` a `MembershipJpaRepository` y al adaptador · repo: backend · ~20 min · depende de T01, T04
