...
- [ ] T04 — Implementa `findByTenantIdAndEmail` en `MembershipJpaRepository` (query derivada) y en `MembershipRepositoryAdapter` (mapea `MembershipEntity` → `Membership` vía `reconstitute`) · repo: backend · ~20 min · depende de T02, T03
- [ ] T05 — `common/security/JwtTokenProvider`: componente Spring que genera un JWT HS256 con claims `sub`/`tenantId`/`email`/`role` y expiración, leyendo `app.jwt.secret`/`app.jwt.expiration-minutes` · repo: backend · ~20 min · depende de T01
- [ ] T06 — Puertos `LoginCommand`, `LoginUseCase`, `LoginResult` (`tenants/domain/port/in`) · repo: backend · ~10 min
